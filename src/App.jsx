...
function Login() {
  ...
  return <main style={{ ...panel, maxWidth: 380, marginTop: 90 }}><h1>Inventory Portal</h1><p>Sign in to manage items.</p><form onSubmit={submit}><label>Username or email<input type="text" value={email} onChange={e => setEmail(e.target.value)} required /></label><label>Password<input type="password" value={password} onChange={e => setPassword(e.target.value)} required /></label>{error && <p role="alert">{error}</p>}<button>Sign in</button></form></main>
}
